...
if ($(".swiper-thumbs-wrap").length) {
    $(".swiper-thumbs-wrap").each(function () {
        const $wrap = $(this);

        const thumbSwiper = new Swiper($wrap.find(".sw-thumbs_media")[0], {
        ...
        const mainSwiper = new Swiper($wrap.find(".sw-thumbs_main")[0], {
            grabCursor: true,
            speed: 800,
            spaceBetween: 16,
            navigation: {
                prevEl: $wrap.find(".nav-prev-swiper")[0],
                nextEl: $wrap.find(".nav-next-swiper")[0],
            },
        });
...
if ($(".slider-thumb-look").length > 0) {
    const $this = $(".slider-thumb-look");
    const $main = $this.find(".swiper-look-main");
    const $thumb = $this.find(".swiper-look-thumb");
    ...
    const thumbSwiper = new Swiper($thumb[0], {
        spaceBetween: 15,
        speed: 800,
        grabCursor: true,

        navigation: {
            prevEl: $this.find(".nav-prev-swiper")[0],
            nextEl: $this.find(".nav-next-swiper")[0],
        },
    });
...
if ($(".modal-quickview").length > 0) {
    var $modalRoot = $(".modal-quickview");
    var mainQV = new Swiper(".modal-quickview .swiper-quickview", {
        slidesPerView: 1,
        spaceBetween: 0,
        observer: true,
        observeParents: true,
        speed: 800,
        navigation: {
            nextEl: ".modal-quickview .single-slide-next",
            prevEl: ".modal-quickview .single-slide-prev",
        },
    });

    function updateModalActiveButton(type, activeIndex) {
        var btnClass = `.${type}_btn`;
        var dataAttr = `data-quick-${type}`;
        var currentClass = `.value-current${capitalizeFirstLetter(type)}`;

        $modalRoot.find(btnClass).removeClass("active");

        var currentSlide = $modalRoot.find(".swiper-slide").eq(activeIndex);
        var currentValue = currentSlide.attr(dataAttr);

        if (currentValue) {
            var $btn = $modalRoot.find(`${btnClass}[${dataAttr}='${currentValue}']`);

            $btn.addClass("active");
            $modalRoot.find(currentClass).text(currentValue);

            if (type === "size") {
                var price = $btn.data("quick-price");
    ...
    function scrollToModalSlide(type, value, color) {
        ...
        var matchingSlides = $modalRoot.find(".swiper-quickview .swiper-slide").filter(function () {
            return $(this).attr(`data-quick-${type}`) === value && $(this).attr("data-quick-color") === color;
        });
        ...
        if (matchingSlides.length > 0) {
            ...
            var fallbackSlides = $modalRoot.find(".swiper-quickview .swiper-slide").filter(function () {
                return $(this).attr(`data-quick-${type}`) === value;
            });
    ...
    function setupModalVariantButtons(type) {
        $modalRoot.find(`.${type}_btn`).on("click", function () {
            var value = $(this).attr(`data-quick-${type}`);
            var color = $modalRoot.find(".value-currentColor").text();

            $modalRoot.find(`.${type}_btn`).removeClass("active");
            $(this).addClass("active");

            if (type === "size") {
                var price = $(this).data("quick-price");
                updateModalPrice(price);
            }
    ...
    function updateModalPrice(price) {
        ...
        var sale = parseFloat(price);
        var old = sale * 1.2;

        $modalRoot.find(".price-on-sale").text(`$${sale.toFixed(2)}`);
        $modalRoot.find(".price-on-old").text(`$${old.toFixed(2)}`);
    }
